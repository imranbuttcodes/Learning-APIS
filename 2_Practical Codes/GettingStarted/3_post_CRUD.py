...
# here order matter why?, it will not work if we put this endpooint after the /posts/{id} endpoint because it will treat "latest" as an id and will not reach this endpoint. So, we need to put this endpoint before the /posts/{id} endpoint.
# @app.get("/posts/latest")
# def get_latest_post():
#     if not POSTS:
#         return None    
